...
export function jsExprToLatex(expr) {
  ...
  // a / b → \frac{a}{b}
  // Операнд — сбалансированный «множитель»: учитывает ^{…}, _{…}, (…) и
  // \команды, поэтому +/- внутри показателя/индекса не обрывают токен (иначе
  // 10^{-10}/10^{-15} рвалось в 10^{-\frac{10}}{10^{}-15}).
  const FRAC_FACTOR = String.raw`(?:\\[a-zA-Z]+(?:\[[^\]]*\]|\{[^{}]*\})*|\([^()]*\)|[0-9A-Za-z.]+)(?:[_^](?:\{[^{}]*\}|\\?[0-9A-Za-z.]+))*`;
  s = s.replace(new RegExp(`(${FRAC_FACTOR})\\s*/\\s*(${FRAC_FACTOR})`, 'g'),
