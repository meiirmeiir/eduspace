...
// Искры вокруг освоенного узла (только mastered).
const SPARKS = [
  { left: '12%', top: '-5px',  dx: '-6px',  dy: '-16px', d: 0   },
  { left: '50%', top: '-7px',  dx: '0px',   dy: '-18px', d: 1.1 },
  { left: '84%', top: '4px',   dx: '8px',   dy: '-15px', d: 0.5 },
  { left: '-5px', top: '42%',  dx: '-14px', dy: '-8px',  d: 1.7 },
  { left: '99%', top: '55%',   dx: '14px',  dy: '-10px', d: 0.3 },
];
...
export default function CustomNode({ data }) {
  const { title, grade, status = 'active', micro_skills = [], mastery = 0, appearDelay = 0 } = data;
  ...
  return (
    <div className={`skill-node skill-node-${vs}`}>
      ...
      <div className="skill-node-appear" style={{ animationDelay: `${appearDelay}s` }}>
        <div className="skill-node-card" style={{
          ...
          {vs === 'mastered' && SPARKS.map((sp, i) => (
            <span key={i} className="mastered-spark" style={{ left: sp.left, top: sp.top, '--dx': sp.dx, '--dy': sp.dy, animationDelay: `${sp.d}s` }} />
          ))}
