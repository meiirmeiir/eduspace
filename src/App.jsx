...
const InputStyle = {
  ...
  background: THEME.bg, fontSize: 15, color: THEME.text, fontFamily: "'Inter', sans-serif",
  transition: "all 0.3s ease", outline: "none", marginBottom: 20
};

const LabelStyle = {
  display: "block", marginBottom: 8, fontSize: 12, fontWeight: 700, color: THEME.textLight, 
...
const Logo = ({ size = 40 }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
    <div style={{ width: size, height: size, borderRadius: '50%', overflow: 'hidden', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
      {/* Здесь картинка подтягивается из папки public. mixBlendMode убирает белый фон */}
      <img src="/logo.jpg" alt="AAPA Logo" style={{ width: '120%', height: '120%', objectFit: 'cover', mixBlendMode: 'multiply' }} />
    </div>
    <div>
      <div style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800, fontSize: size * 0.65, color: THEME.primary, lineHeight: 1, letterSpacing: "1px" }}>AAPA</div>
      <div style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: size * 0.22, color: THEME.accent, letterSpacing: "1.5px", marginTop: 4, textTransform: "uppercase" }}>Ad Astra Per Aspera</div>
    </div>
  </div>
);
      ...
      <div className="split-left">
        ...
        <div>
          <div className="trust-badge">
            <span style={{ color: THEME.accent, letterSpacing: "2px" }}>★★★★★</span>
      ...
      <div className="split-right">
        <div className="form-card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="input-group">
              <label style={LabelStyle}>Цель обучения</label>
            ...
            {mainGoal === 'exam' && (
              <div className="input-group scale-in">
                <label style={LabelStyle}>Какой экзамен?</label>
...
// ── Report Screen (Refactored based on Document) ──────────────────────────────
  ...
  return (
    <>
      <style>{`
        ...
        .form-card { width: 100%; max-width: 460px; background: #fff; padding: 40px; border-radius: 16px; border: 1px solid ${THEME.border}; box-shadow: 0 15px 35px -5px rgba(10,25,47,0.03); }
      ...
      {screen !== "auth" && (
        <nav style={{ background: THEME.surface, borderBottom: `1px solid ${THEME.border}`, padding: "16px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", position: 'sticky', top: 0, zIndex: 1000 }}>
          <Logo size={32} />
          {["report","rpgmap"].includes(screen) && (
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setScreen("report")} style={{ background: screen === 'report' ? THEME.bg : "transparent", border: `1px solid ${screen === 'report' ? THEME.border : 'transparent'}`, color: THEME.primary, padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 13 }}>Отчет</button>
              <button onClick={() => setScreen("rpgmap")} style={{ background: screen === 'rpgmap' ? THEME.primary : "transparent", border: "none", color: screen === 'rpgmap' ? THEME.accent : THEME.textLight, padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 13 }}>Трек</button>
            </div>
          )}
        </nav>
      )}
      ...
      {screen === "question" && (
        <>
          <div style={{ maxWidth: 800, margin: "20px auto 0", padding: "0 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <Logo size={28} />
