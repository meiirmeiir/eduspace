...
// Ядро упаковки: items = [{ block, id? }] → [{ text, ids }]. ids — id-шники
// (childUid), чьи блоки попали в ЭТО сообщение → для per-child идемпотентности
// дайджеста (пометить доставленных при успехе конкретного сообщения). Текст
// собирается ТОЧНО как раньше (склейка блоков по размеру MSG_LIMIT, футер 1 раз).
function packGroups(items, header = null) {
  const groups = [];
  let cur = null, curIds = null;   // тело текущего сообщения (без футера) + его id-шники
  const start = (it) => {
    const head = (groups.length === 0 && header) ? header + '\n\n' : '';
    cur = head + it.block;
    curIds = it.id != null ? [it.id] : [];
  };
  for (const it of items) {
    if (cur === null) { start(it); continue; }
    const candidate = cur + REPORT_SEP + it.block;
    if (candidate.length + REPORT_FOOTER.length > MSG_LIMIT) {
      groups.push({ text: cur + REPORT_FOOTER, ids: curIds });   // закрыть текущее
      start(it);                                                 // новое сообщение (хедер уже не добавится)
    } else {
      cur = candidate;
      if (it.id != null) curIds.push(it.id);
    }
  }
  if (cur !== null) groups.push({ text: cur + REPORT_FOOTER, ids: curIds });
  return groups;
}

// Строковый API (без id) — для /report, /themes, /dynamics. Вывод 1:1 как раньше.
function packMessages(blocks, header = null) {
  return packGroups(blocks.map(b => ({ block: b })), header).map(g => g.text);
}
...
// Группы для дайджеста: [{ text, childUids }] — childUids детей, попавших в
// сообщение (для per-child пометки digestSentWeeks при успешной доставке).
function buildReportGroups(children, header = null) {
  return packGroups(children.map(c => ({ block: childBlock(c.name, c.r), id: c.cid })), header)
    .map(g => ({ text: g.text, childUids: g.ids }));
}
  ...
  async () => {
    ...
    for (const linkDoc of links.docs) {
      ...
      try {
        ...
        if (ld.lastSentWeekId === weekId) continue;        // ВСЯ неделя готова (вкл. legacy-флаг) — fast-path
        ...
        // ── Per-child идемпотентность: digestSentWeeks { childUid: weekId } ──
        // Шлём ТОЛЬКО недосланных за эту неделю → частичный сбой не теряет детей и
        // не помечает неделю ложно-готовой; ре-триггер досылает только недосланных,
        // не дублируя посланных. (lastSentWeekId выше — fast-path/legacy полной недели.)
        const sent = ld.digestSentWeeks || {};
        const pendingUids = childUids.filter(cid => sent[cid] !== weekId);
        if (!pendingUids.length) {                         // все дети уже посланы → включить fast-path впредь
          await linkDoc.ref.set({ lastSentWeekId: weekId }, { merge: true });
          continue;
        }

        // отчёты pending-детей (per-child try/catch — сбойный билд оставляет ребёнка pending)
        const children = [];
        for (const cid of pendingUids) {
          try {
            const r = await getChildReportForWeek(cid, weekId);
            const p = await db.collection('publicProfiles').doc(cid).get();
            const pd = p.exists ? p.data() : {};
            const name = escapeHtml(`${pd.firstName || ''} ${pd.lastName || ''}`.trim() || 'Ребёнок');
            children.push({ cid, name, r });
          } catch (e) {
            logger.error('digest child failed', { parentUid, child: cid, err: String(e) });
          }
        }
        if (!children.length) continue;    // все pending сбоили на билде — НЕ помечаем (досыл при ре-триггере)

        // Сообщения с хедером «Итоги недели»; per-child пометка при успехе КАЖДОГО сообщения.
        let blocked = false;
        const justSent = [];               // childUids, реально доставленные в этом прогоне
        for (const g of buildReportGroups(children, '📅 <b>Итоги недели</b>')) {
          const res = await sendBotMessage(token, chatId, g.text);
          if (!res.ok && res.status === 403) { blocked = true; break; }
          if (res.ok) {                    // пометить детей этого сообщения (инкрементально — переживает краш)
            const upd = {};
            for (const cid of g.childUids) upd[cid] = weekId;
            await linkDoc.ref.set({ digestSentWeeks: upd }, { merge: true });
            justSent.push(...g.childUids);
          }
          // не-403 сбой: дети группы остаются pending (не помечены) → досыл при ре-триггере
        }
        sentChildren += justSent.length;
        ...
        } else if (childUids.every(cid => sent[cid] === weekId || justSent.includes(cid))) {
          // lastSentWeekId — ТОЛЬКО когда ВСЕ дети доставлены (фикс бага «флаг при не-403-сбое»)
          await linkDoc.ref.set({ lastSentWeekId: weekId }, { merge: true });
          sentLinks++;
        }
      } catch (e) {
